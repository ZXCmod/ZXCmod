...
void CBarney ::BarneyFirePistol(void)
{
	...
	vecShootOrigin = pev->origin + Vector(0, 0, 55) + gpGlobals->v_forward * 40;
	Vector vecShootDir = ShootAtEnemy(vecShootOrigin);
	Vector vecThrow = gpGlobals->v_forward * 6000;
...
void CBarney ::TalkInit()
{
	...
	m_voicePitch = RANDOM_FLOAT(70, 120);
}
...
int CBarney ::TakeDamage(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType)
{
	...
	if (m_MonsterState != MONSTERSTATE_PRONE && (pevAttacker->flags & FL_CLIENT))
	{
		...
		if (m_hEnemy == NULL)
		{
			...
			if ((m_afMemory & bits_MEMORY_SUSPICIOUS) || IsFacing(pevAttacker, pev->origin) && pev->health < 150)
			{
...
void CDeadBarney ::Spawn()
{
	...
	pev->health = 200; // gSkillData.barneyHealth;
