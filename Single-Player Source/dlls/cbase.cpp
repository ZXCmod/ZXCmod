...
int DispatchSpawn(edict_t *pent)
{
	...
	if (pEntity)
	{
		...
		if (pEntity)
		{
			...
			// 	pEntity->pev->owner = pEntity->edict();
		}
...
int CBaseEntity ::TakeDamage(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType)
{
	...
	CBasePlayer *pl = (CBasePlayer *)CBasePlayer::Instance(pevAttacker);
	if (pl)
		flDamage *= pl->TripleShot;

	// do the damage
	pev->health -= flDamage * (sqrtf(pev->health)/10);
	...
	if (pev->health <= 0) // if killed :
	{
		...
		// else
		// 	Killed(pevInflictor, GIB_NORMAL);
		return 0;
	}
...
CBaseEntity *CBaseEntity::Create(char *szName, const Vector &vecOrigin, const Vector &vecAngles, edict_t *pentOwner)
{
	...
	CBasePlayer *pl = (CBasePlayer *)CBasePlayer::Instance(pentOwner);
	...
	if (pl && pl->TripleShot > 1)
	{
		pEntity->pev->dmg *= pl->TripleShot;
		pEntity->pev->renderfx = kRenderFxGlowShell;
		pEntity->pev->rendercolor = Vector(250,150,0);
		pEntity->pev->renderamt = 80.0;
	}
