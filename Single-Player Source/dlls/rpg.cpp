...
// void CRpg::SecondaryAttack() // test 
// {
// 	if (m_iClip) 
// 	{
// 		UTIL_MakeVectors(m_pPlayer->pev->v_angle);
...
// 					CBaseEntity::Create("monster_zombie", m_pPlayer->GetGunPosition() + gpGlobals->v_forward * RANDOM_LONG(50,300) + gpGlobals->v_right * 8 + gpGlobals->v_up * -12, vecThrow, m_pPlayer->edict());
// 				//CGib::SpawnRandomGibs(pev, 350, 1);
// #endif
// 				PLAYBACK_EVENT(FEV_GLOBAL, m_pPlayer->edict(), m_usRpg);
					...
					{
						{
							if (m_pPlayer->m_flNextHeavyTuretsTime < gpGlobals->time) // need delay
							{
								...
								Vector vecThrow = gpGlobals->v_forward;
								m_pPlayer->m_flNextTurretsLimit++;
								CBaseEntity::Create("monster_turret", pev->origin, vecThrow, m_pPlayer->edict());
