...
void CSquidSpit :: Touch ( CBaseEntity *pOther )
{
	...
	if ( !pOther->pev->takedamage )
	{
		...
		MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, tr.vecEndPos );
			...
			WRITE_BYTE ( 16 );			// count
			WRITE_BYTE ( 70 );			// speed
			WRITE_BYTE ( 160 );			// noise ( client will divide by 100 )
		MESSAGE_END();
	}
	else
	{
...
{
	if ( gpGlobals->time - m_flLastHurtTime < 1 && FClassnameIs ( pTarget->pev, "monster_headcrab" ) )
	{
...
int CBullsquid :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	// it will swerve. (whew).
	if ( m_hEnemy != NULL && IsMoving() && pevAttacker == m_hEnemy->pev && gpGlobals->time - m_flLastHurtTime > 0.3 )
	{
...
BOOL CBullsquid :: CheckRangeAttack1 ( float flDot, float flDist )
{
	...
	if ( flDist > 64 && flDist <= 784 && flDot >= 0.5 && gpGlobals->time >= m_flNextSpitTime )
	{
		...
		if ( IsMoving() )
		{
			...
			m_flNextSpitTime = gpGlobals->time + 0.7;
		}
		else
		{
			// not moving, so spit again pretty soon.
			m_flNextSpitTime = gpGlobals->time + 0.1;
		}
...
{
	if ( m_hEnemy->pev->health <= 14 && flDist <= 85 && flDot >= 0.7 )
	{
		return TRUE;
	}
	return FALSE;
...
void CBullsquid :: SetYawSpeed ( void )
{
	...
	switch ( m_Activity )
	{
	case	ACT_WALK:			ys = 130;	break;
	case	ACT_RUN:			ys = 130;	break;
	case	ACT_IDLE:			ys = 130;	break;
	case	ACT_RANGE_ATTACK1:	ys = 130;	break;
	default:
		ys = 190;
		break;
	}
...
void CBullsquid :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	switch( pEvent->event )
	{
		case BSQUID_AE_SPIT:
		{
			...
			vecSpitDir.x += RANDOM_FLOAT( -0.1, 0.1 );
			...
			CSquidSpit::Shoot( pev, vecSpitOffset, vecSpitDir * 800 );
		}
		break;
		...
		{
			// SOUND HERE!
			CBaseEntity *pHurt = CheckTraceHullAttack( 60, 30, DMG_SLASH );
		...
		{
			CBaseEntity *pHurt = CheckTraceHullAttack( 70, 35, DMG_CLUB | DMG_ALWAYSGIB );
			if ( pHurt ) 
			{
				pHurt->pev->punchangle.z = -20;
				pHurt->pev->punchangle.x = 20;
				pHurt->pev->velocity = pHurt->pev->velocity + gpGlobals->v_right * 400;
...
//			pev->velocity.z += (0.875 * flGravity) * 0.5;
			pev->velocity.z += (1.225 * flGravity) * 0.5;
		}
		break;
