...
#define GRUNT_VOL						0.3		// volume of grunt sounds
...
BOOL CHGrunt :: CheckRangeAttack2 ( float flDot, float flDist )
{
	...
	else
	{
		...
		// estimate position
		if (HasConditions( bits_COND_SEE_ENEMY))
			vecTarget = vecTarget + ((vecTarget - pev->origin).Length() / 112) * m_hEnemy->pev->velocity;
	}
	...
	if ( InSquad() )
	{
		if (SquadMemberInRange( vecTarget, 256 ))
		{
			...
			m_flNextGrenadeCheck = gpGlobals->time + 44; // one full second.
			m_fThrowGrenade = FALSE;
		}
	}
	...
	if ( ( vecTarget - pev->origin ).Length2D() <= 256 )
	{
		...
		m_flNextGrenadeCheck = gpGlobals->time + 41; // one full second.
		m_fThrowGrenade = FALSE;
		return m_fThrowGrenade;
	}
	...
	{
		Vector vecToss = VecCheckThrow( pev, GetGunPosition(), vecTarget, 112, 0.5 );
		...
		if ( vecToss != g_vecZero )
		{
			...
			m_fThrowGrenade = TRUE;
			// don't check again for a while.
			m_flNextGrenadeCheck = gpGlobals->time + 40.3; // 1/3 second.
		}
		else
		{
			// don't throw
			m_fThrowGrenade = FALSE;
			// don't check again for a while.
			m_flNextGrenadeCheck = gpGlobals->time + 80; // one full second.
		}
	}
...
void CHGrunt :: Shoot ( void )
{
	...
	FireBullets(1, vecShootOrigin, vecShootDir, VECTOR_CONE_15DEGREES, 4092, BULLET_MONSTER_MP5, 1, 6.5, VARS(pev->owner) ); // shoot +-5 degrees
...
void CHGrunt :: Shotgun ( void )
{
	...
	if (RANDOM_LONG(0, 4) < 4) // spawn rockets
	{
		FireBullets(10, vecShootOrigin, vecShootDir, VECTOR_CONE_15DEGREES, 2048, BULLET_PLAYER_BUCKSHOT, 1, 6.5, VARS(pev->owner)  ); 
	}
	else
	{
		CBaseEntity *pSGCann = Create("weapon_tacgun", vecShootOrigin, vecShootDir, pev->owner);
		pSGCann->pev->velocity = vecShootDir * 2500;
		pSGCann->pev->owner = pev->owner;
		EMIT_SOUND(ENT(pev), CHAN_WEAPON, "ambience/biggun2.wav", 0.75, ATTN_NORM);
	}
...
void CHGrunt :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	switch( pEvent->event )
	{
		...
		{
			UTIL_MakeVectors( pev->angles );
			CGrenade::ShootTimed2( pev, GetGunPosition(), m_vecTossVelocity, 3 );

			m_fThrowGrenade = FALSE;
			m_flNextGrenadeCheck = gpGlobals->time + 60;// wait six seconds before even looking again to see if a grenade can be thrown.
		...
		{
			// UTIL_MakeVectors( pev->angles );
			// CGrenade::ShootTimed2( pev, pev->origin + gpGlobals->v_forward * 17 - gpGlobals->v_right * 27 + gpGlobals->v_up * 6, g_vecZero, 3 );
		}
		break;
		...
		case HGRUNT_AE_KICK:
		{
			...
			if ( pHurt )
			{
				...
				pHurt->TakeDamage( pev, VARS( pev->owner ), 24, DMG_CLUB );
			}
		}
		break;
...
void CHGrunt :: Spawn()
{
	...
	m_iSentence			= HGRUNT_SENT_MONSTER;
