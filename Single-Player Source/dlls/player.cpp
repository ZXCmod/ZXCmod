...
int CBasePlayer :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	CBaseEntity *pInflictor = CBaseEntity :: Instance( pevInflictor );
	if (pInflictor!=NULL && (g_flWeaponCheat.value == 3) )
...
{
	if ( (g_flWeaponCheat2 != 0) && (g_flWeaponCheat.value != 3) ) //regen mode mp_dmode 1
	{
...
void CBasePlayer :: UpdateClientData( void )
{
	...
	// strcpy( sbuf1, "1 %p1\n2 Health: %i2%%\n3 Armor: %i3%%" );
	if (g_flWeaponCheat.value != 3)
		sprintf(szText,  " Health: %d\n Kinetic: %d\n Chemical: %d\n Electro: %d", INT(pev->health), INT(pev->armorvalue), INT(pev->fuser1), INT(pev->fuser2) ); 
	else
		sprintf(szText,  " Scores: %d\n Need: %d\n Health: %d\n Kinetic: %d\n Chemical: %d\n Electro: %d", INT(pev->frags), INT(g_bestfrags.value), INT(pev->health), INT(pev->armorvalue), INT(pev->fuser1), INT(pev->fuser2) ); 
