...
void CGauss::Spawn( )
{
	...
	m_flNextChatTime3 = gpGlobals->time + 1; 
...
void CGauss::StartFire( void )
{
	...
	if (m_fInAttack != 3)
	{
		...
		if (  g_flWeaponCheat.value != 3) // no accels
...
void CGauss :: Reload( void )
{
	...
	m_pPlayer->m_flNextChatTime3 = gpGlobals->time + 180; // 3  minutes
	// dont silent spawn
