...
void CStomp::Think( void )
{
	...
	if ( tr.pHit && tr.pHit != pev->owner )
	{
		...
		if ( pEntity )
			pEntity->TakeDamage( pev, pevOwner, 45, DMG_BULLET );
	}
...
void CGargantua :: FlameUpdate( void )
{
	...
	for ( i = 0; i < 2; i++ )
	{
		if ( m_pFlame[i] )
		{
			...
			FlameDamage( vecStart, trace.vecEndPos, pev, pev, 7, CLASS_ALIEN_MONSTER, DMG_BURN );
	...
	while ((pEntity = UTIL_FindEntityInSphere( pEntity, vecMid, searchRadius )) != NULL)
	{
		if ( pEntity->pev->takedamage != DAMAGE_NO )
		{
			...
			{// the explosion can 'see' this entity, so hurt them!
				...
				{
					ClearMultiDamage( );
					pEntity->TraceAttack( pevInflictor, flAdjustedDamage/3, (tr.vecEndPos - vecSrc).Normalize( ), &tr, bitsDamageType );
...
void CGargantua::HandleAnimEvent(MonsterEvent_t *pEvent)
{
	...
	case GARG_AE_SLASH_LEFT:
		{
			// HACKHACK!!!
			CBaseEntity *pHurt = GargantuaCheckTraceHullAttack( GARG_ATTACKDIST + 10.0, 37, DMG_BLAST );
			if (pHurt)
			{
	...
	case GARG_AE_STOMP:
		//StompAttack();
		EMIT_SOUND_DYN ( edict(), CHAN_VOICE, pBreatheSounds[ RANDOM_LONG(0,ARRAYSIZE(pBreatheSounds)-1) ], 1.0, ATTN_GARG, 0, PITCH_NORM + RANDOM_LONG(-30,0) );
