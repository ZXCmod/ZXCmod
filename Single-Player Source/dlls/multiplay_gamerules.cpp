...
CHalfLifeMultiplay ::CHalfLifeMultiplay()
{
	...
	g_flMSpawnTime = gpGlobals->time + 0.1;		
...
void CHalfLifeMultiplay ::Think(void)
{
	...
	if (g_fGameOver) // someone else quit the game already
	{
		...
		if (time < 1)
			CVAR_SET_STRING("mp_chattime", "3");
		else if (time > MAX_INTERMISSION_TIME)
			CVAR_SET_STRING("mp_chattime", UTIL_dtos1(MAX_INTERMISSION_TIME));

		m_flIntermissionEndTime = g_flIntermissionStartTime + 3;
...
void    CHalfLifeMultiplay :: MSpawnUpdate( )
{
	
	if (gpGlobals->time >= g_flMSpawnTime)
	{
		if (MonSpawnCounter < g_zxc_mspawn.value)
		{
			CBaseEntity *pEntity = NULL;
			CBaseEntity *pMonstro = NULL;
			Vector VRand = Vector(RANDOM_LONG(-4096, 4096),RANDOM_LONG(-4096, 4096),RANDOM_LONG(-4096, 4096));
			...
			while ((pEntity = UTIL_FindEntityInSphere(pEntity, VRand , 1500)) != NULL)
			{
				if (pEntity->pev->takedamage && !pEntity->IsPlayer() && !pEntity->IsMonster())
				{
					UTIL_TraceLine(pEntity->pev->origin, pEntity->pev->origin + Vector(0,0,1024) + (Vector(RANDOM_FLOAT(-1.25, 1.25),RANDOM_FLOAT(-1.25, 1.25),0) * 512), ignore_monsters, ENT(pEntity->pev), &tr);
					//UTIL_TraceHull(tr.vecEndPos, tr.vecEndPos - Vector(0,0,2048) + (Vector(RANDOM_FLOAT(-1.25, 1.25),RANDOM_FLOAT(-1.25, 1.25),0) * 512), ignore_monsters, human_hull, ENT(pEntity->pev), &tr2);
					UTIL_TraceLine(tr.vecEndPos, tr.vecEndPos - Vector(0,0,2048) + (Vector(RANDOM_FLOAT(-1.25, 1.25),RANDOM_FLOAT(-1.25, 1.25),0) * 512), ignore_monsters, ENT(pEntity->pev), &tr2);
					UTIL_TraceHull(tr2.vecEndPos + Vector(0,0,45), tr2.vecEndPos - Vector(0,0,25), dont_ignore_monsters, head_hull, ENT(pEntity->pev), &tr3);
					
					if ( !tr3.fStartSolid)
					{
						m_TEnd = ( ( tr2.vecEndPos) + (tr2.vecPlaneNormal * 16) ); 

						switch (RANDOM_LONG(0, 7))
						{
						case 0:
							pMonstro = CBaseEntity::Create("monster_human_grunt", m_TEnd, pEntity->pev->angles, pEntity->edict()); 
							break;
						case 1:
							pMonstro = CBaseEntity::Create("monster_human_assassin", m_TEnd, pEntity->pev->angles, pEntity->edict()); 
							break;
						case 2:
							pMonstro = CBaseEntity::Create("monster_gman", m_TEnd, pEntity->pev->angles, pEntity->edict()); 
							break;
						case 3:
							pMonstro = CBaseEntity::Create("monster_ichthyosaur", m_TEnd, pEntity->pev->angles, pEntity->edict()); 
							break;
						case 4:
							pMonstro = CBaseEntity::Create("monster_bullchicken", m_TEnd, pEntity->pev->angles, pEntity->edict()); 
							break;
						case 5:
							pMonstro = CBaseEntity::Create("monster_zombie", m_TEnd, pEntity->pev->angles, pEntity->edict());  
							break;
						case 6:
							pMonstro = CBaseEntity::Create("monster_headcrab", m_TEnd, pEntity->pev->angles, pEntity->edict()); 
							break;
						case 7:
							pMonstro = CBaseEntity::Create("monster_miniturret", m_TEnd, pEntity->pev->angles, pEntity->edict()); 
							pMonstro->pev->ltime = RANDOM_LONG(0, 8);
							pMonstro->Classify2 = CLASS_HUMAN_MILITARY;
							break;
						}
						...
						// MESSAGE_BEGIN(MSG_BROADCAST, SVC_TEMPENTITY); // debug line
						...
						// WRITE_SHORT(g_sModelIndexSmokeTrail);		   // sprite
						...
						pMonstro->pev->angles.x = 0.0;
						pMonstro->pev->angles.y = 0.0;
						pMonstro->pev->iuser4 = 1;
						pMonstro->pev->flags &= ~FL_ONGROUND;
						MonSpawnCounter++;
						break;
					}
				}
		...
		MCounterUpdate( );
		g_flMSpawnTime = gpGlobals->time + 0.05;	
	}
	
	
}

void    CHalfLifeMultiplay :: MCounterUpdate( )
{
	// >1.38 entity counter
	CBaseEntity *pEntity = NULL;
	int X = 0;
...
void CHalfLifeMultiplay ::InitHUD(CBasePlayer *pl)
{
	...
	{
	case 0:
		txt = "Official group zxc-mod: vk.com/zxcmod  ";
		break;
	// case 1: txt = "More about zxc-mod on the ofsite: www.ZxcMod.com  "; break;
	case 1:
		txt = "More about zxc-mod on the ofsite: vk.com/zxcmod  ";
		break;
	case 2:
		txt = "zxc-mod rpynna: www.vkontakte.ru/zxcmod ";
		break;
	}
...
void CHalfLifeMultiplay ::ClientDisconnected(edict_t *pClient)
{
	if (pClient != NULL)
	{
		...
		if (pPlayer)
		{
			...
			edict_t *pFind; // thanks to bubblemod
			...
			while ( !FNullEnt( pFind ) )
			{
				...
				if ( pTrip )
				{
					if ( pTrip->Owner() == pPlayer->edict() )
					{
						pTrip->TakeDamage(pTrip->pev, pTrip->pev, 1000, 1);
					}
				}
...
void CHalfLifeMultiplay ::PlayerSpawn(CBasePlayer *pPlayer)
{
	...
	if (g_zxc_cheats.value > 0)
	{
		if (g_zxc_promode.value == 0)
		{
			...
			pPlayer->GiveAmmo(100, "357", _357_MAX_CARRY);
			pPlayer->GiveAmmo(100, "buckshot", BUCKSHOT_MAX_CARRY);
			pPlayer->GiveAmmo(3, "rockets", ROCKET_MAX_CARRY);
			pPlayer->GiveAmmo(3, "Satchel Charge", SATCHEL_MAX_CARRY);
			pPlayer->GiveAmmo(10, "ARgrenades", M203_GRENADE_MAX_CARRY);
		...
		else
		{
			...
			pPlayer->GiveAmmo(3, "rockets", ROCKET_MAX_CARRY);
			pPlayer->GiveAmmo(10, "ARgrenades", M203_GRENADE_MAX_CARRY);
			pPlayer->GiveAmmo(50, "uranium", URANIUM_MAX_CARRY);
		}
	}
	else if (addDefault)
	{
...
void CHalfLifeMultiplay ::PlayerKilled(CBasePlayer *pVictim, entvars_t *pKiller, entvars_t *pInflictor)
{
	...
	if (peKiller)
	{
		...
		else if (ktmp && ktmp->IsPlayer())
		{
			...
			if (g_zxc_cheats.value == 1)
			{
				peKiller->GiveAmmo(50, "9mm", _9MM_MAX_CARRY);
				peKiller->GiveAmmo(6, "357", _357_MAX_CARRY);
				peKiller->GiveAmmo(8, "buckshot", BUCKSHOT_MAX_CARRY);
				peKiller->GiveAmmo(5, "bolts", BOLT_MAX_CARRY);
				peKiller->GiveAmmo(2, "rockets", ROCKET_MAX_CARRY);
				peKiller->GiveAmmo(2, "ARgrenades", M203_GRENADE_MAX_CARRY);
				peKiller->GiveAmmo(20, "uranium", URANIUM_MAX_CARRY);
			}

			FireTargets("game_playerkill", ktmp, ktmp, USE_TOGGLE, 0);

			// if ( peKiller->m_pActiveItem->m_iId == WEAPON_CROWBAR) // can crash
			{
				//if (peKiller->pev->max_health < 700.0)
				peKiller->pev->max_health += 10.0;
				peKiller->pev->health += 10.0;
				peKiller->pev->armorvalue += 1.0;
				peKiller->pev->fuser1 += 1.0;
			}
		}
		else
		{ 
			pVictim->pev->frags -= 1; // killed by the world
		}
	}
	// update the scores
	// killed scores
