...
void CAGrunt ::PrescheduleThink(void)
{
	if (ShouldSpeak())
	{
		if (m_flNextWordTime < gpGlobals->time)
		{
			...
			// play a new sound
			EMIT_SOUND(ENT(pev), CHAN_VOICE, pIdleSounds[num], 0.5, ATTN_NORM);
...
void CAGrunt ::DeathSound(void)
{
	...
	EMIT_SOUND(ENT(pev), CHAN_VOICE, pDieSounds[RANDOM_LONG(0, ARRAYSIZE(pDieSounds) - 1)], 0.5, ATTN_NORM);
}
...
void CAGrunt ::AlertSound(void)
{
	...
	EMIT_SOUND(ENT(pev), CHAN_VOICE, pAlertSounds[RANDOM_LONG(0, ARRAYSIZE(pAlertSounds) - 1)], 0.5, ATTN_NORM);
}
...
void CAGrunt ::AttackSound(void)
{
	...
	EMIT_SOUND(ENT(pev), CHAN_VOICE, pAttackSounds[RANDOM_LONG(0, ARRAYSIZE(pAttackSounds) - 1)], 0.5, ATTN_NORM);
}
...
void CAGrunt ::PainSound(void)
{
	...
	EMIT_SOUND(ENT(pev), CHAN_VOICE, pPainSounds[RANDOM_LONG(0, ARRAYSIZE(pPainSounds) - 1)], 0.5, ATTN_NORM);
}
...
void CAGrunt ::HandleAnimEvent(MonsterEvent_t *pEvent)
{
	...
	case AGRUNT_AE_HORNET5:
	{
		...
		vecArmPos = vecArmPos + vecDirToEnemy * 30;
		...
		vecDir = ShootAtEnemy( vecArmPos );
		float deflection = 0.03;
		vecDir = vecDir + gpGlobals->v_right * RANDOM_FLOAT( -deflection, deflection ) + gpGlobals->v_up * RANDOM_FLOAT( -deflection, deflection );
		UTIL_TraceLine(vecArmPos, vecArmPos + vecDir * 4092, dont_ignore_monsters, edict(), &tr);
		...
		WRITE_SHORT(iAgruntMuzzleFlash); // sprite
		WRITE_BYTE(0);	   // Starting frame
		WRITE_BYTE(12);	   // framerate * 0.1
		WRITE_BYTE(2);	   // life * 0.1
		WRITE_BYTE(12);	   // width
		WRITE_BYTE(1);	   // noise
		WRITE_BYTE(125);   // color r,g,b
		WRITE_BYTE(65);	   // color r,g,b
		WRITE_BYTE(222);	   // color r,g,b
		WRITE_BYTE(222);   // brightness
		WRITE_BYTE(100);   // scroll speed
		MESSAGE_END();

		UTIL_Sparks( tr.vecEndPos );
		EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "weapons/electro4.wav", 0.55, ATTN_NORM, 0, RANDOM_LONG(130, 160));
		CBaseEntity *pEntity = CBaseEntity::Instance(tr.pHit);

		if (pEntity && pEntity->pev->takedamage > 0 && pEntity->pev->movetype != MOVETYPE_NONE)
		{
			
			pEntity->TakeDamage(pev, VARS( pev->owner ), RANDOM_FLOAT(3.0, 27.0), DMG_ENERGYBEAM);


		}

		// CBaseEntity *pHornet = CBaseEntity::Create("hornet", vecArmPos, UTIL_VecToAngles(vecDirToEnemy), pev->owner);
		// UTIL_MakeVectors(pHornet->pev->angles);
		// pHornet->pev->velocity = gpGlobals->v_forward * 200;

		// switch (RANDOM_LONG(0, 2))
		// {
		// case 0:
		// 	EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "agrunt/ag_fire1.wav", 0.7, ATTN_NORM, 0, 100);
		// 	break;
		// case 1:
		// 	EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "agrunt/ag_fire2.wav", 0.7, ATTN_NORM, 0, 100);
		// 	break;
		// case 2:
		// 	EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "agrunt/ag_fire3.wav", 0.7, ATTN_NORM, 0, 100);
		// 	break;
		// }
	...
	{
		CBaseEntity *pHurt = CheckTraceHullAttack(AGRUNT_MELEE_DIST, 35, DMG_CLUB);
	...
	{
		CBaseEntity *pHurt = CheckTraceHullAttack(AGRUNT_MELEE_DIST, 25, DMG_CLUB);
...
void CAGrunt ::Spawn()
{
	...
	m_flNextSpeakTime = m_flNextWordTime = gpGlobals->time + RANDOM_LONG(0, 10);
...
BOOL CAGrunt ::CheckRangeAttack1(float flDot, float flDist)
{
	...
	if (HasConditions(bits_COND_SEE_ENEMY) && flDist <= 6000 && flDot >= 0.5 && NoFriendlyFire())
	{
		TraceResult tr;
		Vector vecArmPos, vecArmDir;
