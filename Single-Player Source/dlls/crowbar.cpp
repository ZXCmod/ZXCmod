...
{
	Precache();
	m_flNextCrowbarReloadTime = gpGlobals->time;
...
int CCrowbar::AddToPlayer(CBasePlayer *pPlayer)
{
	if (CBasePlayerWeapon::AddToPlayer(pPlayer))
	{
		...
		pPlayer->m_rgAmmo[PrimaryAmmoIndex()] = 1;
...
void CCrowbar::SecondaryAttack()
{
	...
	if (g_zxc_promode.value == 0 && m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] >= 3)
	{
			...
			if (!Swing(1))
			{
				...
				//CBaseEntity *Linr = CBaseEntity::Create("weapon_rocketlinear", GunPosition, m_pPlayer->pev->v_angle, m_pPlayer->edict());
				CRocketlinear* Linr = CRocketlinear :: Create( GunPosition,m_pPlayer->pev->v_angle,m_pPlayer );
				Linr->pev->velocity = Linr->pev->velocity + gpGlobals->v_right;
				Linr->pev->velocity = Linr->pev->velocity + gpGlobals->v_up;
...
void CCrowbar::ThirdAttack()
{
	...
	UTIL_SetOrigin(m_pPlayer->pev, VARS(pentTarget)->origin + Vector(0, 0, 1));
	m_pPlayer->pev->angles = pentTarget->v.angles;

	if (m_pPlayer->IsPlayer())
	{
		m_pPlayer->pev->v_angle = pentTarget->v.angles;
	}
	...
	// if (m_pPlayer->ParalyzeTime > 0) // reset paralize
	// 	m_pPlayer->ParalyzeTime = 0;
...
int CCrowbar::Swing(int fFirst)
{
	...
	PLAYBACK_EVENT_FULL(FEV_GLOBAL, m_pPlayer->edict(), m_usCrowbar,
	...
	else
	{
		...
		pev->dmg = 25.0 * m_pPlayer->TripleShot;
		...
		if (pEntity)
		{
			if (pEntity->Classify() != CLASS_NONE && pEntity->Classify() != CLASS_MACHINE)
			{
				...
				if (m_touch == 1)
				{
					pEntity->pev->velocity = gpGlobals->v_forward * 800; // relative by player, well done 1.31
					m_flNextPrimaryAttack = m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + 0.5;
				}
			}
		}
		if (fHitWorld)
		{
			if (m_touch == 1)
			{
				m_pPlayer->pev->velocity = m_pPlayer->pev->velocity - gpGlobals->v_forward * 800;
...
void CCrowbar::WeaponIdle(void)
{
	...
	if (m_pPlayer->pev->button & IN_RELOAD && g_zxc_promode.value == 0 && m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] >= 6) // chaos rocket
	{
		if (m_pPlayer->m_flNextCrowbarReloadTime < gpGlobals->time)
		{
			Vector GunPosition = m_pPlayer->GetGunPosition();
			GunPosition = GunPosition + gpGlobals->v_forward * 0;
			GunPosition = GunPosition + gpGlobals->v_right * 7;
			...
			// CBaseEntity::Create("rocket_chaos", GunPosition, vecThrow, m_pPlayer->edict());
			// if (m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] >= 16)
			// {
			// 	CBaseEntity::Create("rocket_chaos", GunPosition + Vector(-20, 0, 0), vecThrow, m_pPlayer->edict());
			// 	CBaseEntity::Create("rocket_chaos", GunPosition + Vector(20, 0, 0), vecThrow, m_pPlayer->edict());
			// }

			Crowbar_rocket_chaos :: Create( GunPosition,
														 m_pPlayer->pev->v_angle,
														 m_pPlayer );

			if (m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] >= 16)
			{
				Crowbar_rocket_chaos :: Create( GunPosition + Vector(-30, 0, -10),
														 m_pPlayer->pev->v_angle,
														 m_pPlayer );
			...
			m_pPlayer->m_flNextCrowbarReloadTime = gpGlobals->time + 0.75;
			m_flNextPrimaryAttack = UTIL_WeaponTimeBase() + 0.75;
			m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + 0.75;
...
void CRocketlinear ::Spawn()
{
	...
	SetThink(MoveThink);
	SetTouch(Hit);
	pev->velocity = gpGlobals->v_forward * 854;
	pev->angles.x = -(pev->angles.x);
	
	pev->nextthink = gpGlobals->time + 0.1;
	pev->dmg = 100.0; // 90 old
...
void CRocketlinear ::Hit(CBaseEntity *Target)
{
	TraceResult TResult;
...
void CRocketlinear ::Explode(TraceResult *TResult, int DamageType)
{
	...
	::RadiusDamage(pev->origin, pev, VARS(pev->owner), pev->dmg, pev->dmg*2, CLASS_NONE, DMG_CRUSH); // DMG

	UTIL_Remove(this);
}

CRocketlinear *CRocketlinear ::Create(Vector Pos, Vector Aim, CBaseEntity *Owner)
{
	CRocketlinear *LinearR = GetClassPtr((CRocketlinear *)NULL);

	UTIL_SetOrigin(LinearR->pev, Pos);
	LinearR->pev->angles = Aim;
	LinearR->Spawn();
	LinearR->SetTouch(CRocketlinear ::Hit);
	LinearR->pev->owner = Owner->edict();
	return LinearR;
}

void CRocketlinear ::MoveThink()
{
...
void Crowbar_rocket_chaos ::Spawn()
{
	Precache();
	SET_MODEL(ENT(pev), "models/rpgrocket.mdl");
	pev->movetype = MOVETYPE_FLY;
	...
	WRITE_BYTE(2);				   // width
	WRITE_BYTE(255);			   // r, g, b
	WRITE_BYTE(RANDOM_LONG(0, 250));			   // r, g, b
	WRITE_BYTE(0);			   // r, g, b
	...
	//pev->velocity = (VARS(pev->owner)->velocity*2) + gpGlobals->v_forward * 200.0; //*350
}

void Crowbar_rocket_chaos ::Precache()
{
	m_iSpriteTexture = PRECACHE_MODEL("sprites/shock.spr");
	m_iSpriteTexture2 = PRECACHE_MODEL("sprites/cexplo.spr");
}

void Crowbar_rocket_chaos ::Hit(CBaseEntity *Target)
{
	TraceResult TResult;
	Vector StartPosition;
	pev->enemy = Target->edict();
...
void Crowbar_rocket_chaos ::Explode(TraceResult *TResult, int DamageType)
{
	...
	WRITE_COORD(pev->origin.x);
	WRITE_COORD(pev->origin.y);
	WRITE_COORD(pev->origin.z + 1200); // reach damage radius over .2 seconds
	WRITE_SHORT(m_iSpriteTexture);
	WRITE_BYTE(0);	 // startframe
	WRITE_BYTE(0);	 // framerate
	WRITE_BYTE(2);	 // life
	...
	::RadiusDamage(pev->origin, pev, VARS(pev->owner), pev->dmg, pev->dmg*1.5, CLASS_NONE, DMG_CRUSH); // explDMG
	//::RadiusDamage( pev->origin, pev, VARS( pev->owner ), pev->dmg*2, 30, CLASS_NONE, DMG_BULLET  ); //directalDMG

	UTIL_Remove(this);
}

Crowbar_rocket_chaos *Crowbar_rocket_chaos ::Create(Vector Pos, Vector Aim, CBaseEntity *Owner)
{
	Crowbar_rocket_chaos *ChaosR = GetClassPtr((Crowbar_rocket_chaos *)NULL);

	UTIL_SetOrigin(ChaosR->pev, Pos);
	ChaosR->pev->angles = Aim;
	ChaosR->Spawn();
	ChaosR->SetTouch(Crowbar_rocket_chaos ::Hit);
	ChaosR->pev->owner = Owner->edict();
	ChaosR->pev->velocity = (VARS(ChaosR->pev->owner)->velocity*1.5) + gpGlobals->v_forward * 350.0; //*350
	return ChaosR;
}

void Crowbar_rocket_chaos ::MoveThink()
{
	...
	// player have lost
	// if (!pl->IsAlive() && pev->renderamt == 100.0)
	// {
	// 	m_flDie = 10; // add more live to rocket
	// 	m_moving = 1;
	// 	pev->renderamt = 45.0;
	// 	pev->rendercolor.y = 75.0; // green
	// 	pev->rendercolor.z = 75.0; // blue
	// 	STOP_SOUND(ENT(pev), CHAN_VOICE, "weapons/rocket1.wav");
	// }
	...
	if (gpGlobals->time >= m_flDie) // time out 15 s
	{
		...
		::RadiusDamage(pev->origin, pev, VARS(pev->owner), pev->dmg*2.0, pev->dmg, CLASS_NONE, DMG_CRUSH); // DMG
		UTIL_Remove(this);
	}

	if  (RANDOM_LONG(0, 500) == 0)
	{
		m_moving = 1;
	}
	...
	if (m_moving == 0)
	{
		...
		vecSpitDir.y = 0;
		vecSpitDir.z = 0;
		pev->velocity.x = (pev->velocity.x + RANDOM_LONG(-150, 150));
	...
	else if (pl != NULL)
	{
		...
		vecDir = pl->pev->origin - pev->origin;

		if (vecDir.Length() < 30.0)
		{
			m_flDie = 0;
		}
		...
		vecTarget = vecDir;
		pev->velocity = pev->velocity * 0.2 + vecTarget * 490.0; // fly to owner
