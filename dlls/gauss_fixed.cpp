...
void    CBlaster2Beam :: Spawn( )
{
        ...
        pev->movetype = MOVETYPE_BOUNCE; //So gravity affects it a *tad*
        pev->solid = SOLID_BBOX;

        UTIL_SetSize( pev, Vector(-1,-1,-1), Vector(1,1,1) );//Point sized bounding box
        UTIL_SetOrigin( pev, pev->origin );
        pev->classname = MAKE_STRING( "nuke" );//PROJectile from BLASTER
...
//////blyya						
if (gpGlobals->time >= m_flDie - 1.1)
	{				
	dmge = pev->dmg = RANDOM_LONG(89,103);
	rad = 3072;
	}
	...
	{
	//dmg = pev->dmg = RANDOM_FLOAT(34, 64);
...
//::RadiusDamage( pev->origin, pev, pevOwner, pev->dmg, 1024, CLASS_NONE, DMG_FREEZE  );
	...
	MESSAGE_BEGIN( MSG_BROADCAST, SVC_TEMPENTITY );
		...
		WRITE_COORD( pev->origin.y );
		WRITE_COORD( pev->origin.z );
		WRITE_COORD( RANDOM_LONG( 64, 128 ) );	// radius
		WRITE_BYTE( RANDOM_LONG( 200, 255 ) );	// R
		WRITE_BYTE( RANDOM_LONG( 200, 255 ) );	// G
		WRITE_BYTE( 1 );	// B
		WRITE_BYTE( RANDOM_LONG( 128, 255 ) );	// life * 10
		WRITE_COORD( 0 ); // decay
	MESSAGE_END();	
*/
	//lights
	...
	MESSAGE_BEGIN( MSG_BROADCAST, SVC_TEMPENTITY );
		...
		WRITE_COORD( pev->origin.y );
		WRITE_COORD( pev->origin.z );
		WRITE_COORD( RANDOM_LONG( 450, 1050 ) );	// radius
		WRITE_BYTE( RANDOM_LONG( 200, 255 ) );	// R
		WRITE_BYTE( RANDOM_LONG( 200, 255 ) );	// G
		WRITE_BYTE( 1 );	// B
		WRITE_BYTE( RANDOM_LONG( 450, 500 ) );	// life * 10
...
// write_byte(decay rate in 10's)
	...
	//RadiusDamage( pev->origin, pev, pev, 500, CLASS_NONE, DMG_BLAST );
...
void    CBlaster2Beam :: Explode2( TraceResult* TResult, int DamageType )
{
	...
	/*
		...
		RadiusDamage( pev->origin, pev, pev, dmge, CLASS_NONE, DMG_SLASH|DMG_CRUSH|DMG_MORTAR );
*/
		MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, pev->origin );
			WRITE_BYTE( TE_EXPLOSION);		// This just makes a dynamic light now
...
//RadiusDamage( pev->origin, pev, pev, dmg, CLASS_NONE, DMG_BLAST );
