	...
	if ( pOther->pev->takedamage && m_flNextAttack < gpGlobals->time )
	{
		...
		if (tr.pHit == pOther->edict())
		{
			...
			if (tr.pHit->v.modelindex != pev->modelindex)
			{
				...
				ClearMultiDamage( );
				pOther->TraceAttack(pev, RANDOM_LONG(7,19), gpGlobals->v_forward, &tr, DMG_SLASH ); 
				if (m_hOwner != NULL)
					ApplyMultiDamage( pev, m_hOwner->pev );
				else
					ApplyMultiDamage( pev, pev );

				pev->dmg += RANDOM_FLOAT(7, 14); // add more explosion damage
...
void CSqueak::PrimaryAttack()
{
	...
	flags = 0;
		...
		if ( tr.fAllSolid == 0 && tr.fStartSolid == 0 && tr.flFraction > 0.25 )
		{
			...
			pSqueak->pev->velocity = gpGlobals->v_forward * 800 + m_pPlayer->pev->velocity;
#endif
...
void CSqueak::SecondaryAttack( void )
{
	...
	flags = 0;
		...
		if ( tr.fAllSolid == 0 && tr.fStartSolid == 0 && tr.flFraction > 0.25 )
		{
			...
			pSqueak->pev->velocity = gpGlobals->v_forward * 240 + m_pPlayer->pev->velocity;
#endif
...
int CSqueakGrenade2 :: Classify ( void )
{
	if (m_iMyClass != 0)
		return m_iMyClass; // protect against recursion
...
void CSqueakGrenade2 :: Spawn( void )
{
	...
	// +BubbleMod
		...
		WRITE_BYTE( 255 ); // r, g, b
		WRITE_BYTE( 1 ); // r, g, b
		WRITE_BYTE( RANDOM_FLOAT(200, 255) ); // brightness
...
void CSqueakGrenade2 :: Killed( entvars_t *pevAttacker, int iGib )
{
	...
	if (m_hOwner != NULL)
		RadiusDamage ( pev, m_hOwner->pev, 10, CLASS_NONE, DMG_POISON );
	else
		RadiusDamage ( pev, pev, 10, CLASS_NONE, DMG_POISON );
...
void CSqueakGrenade2::HuntThink( void )
{
	...
	// higher pitch as squeeker gets closer to detonation time
	float flpitch = 95.0 - 70.0 * ((m_flDie - gpGlobals->time) / SQUEEK_DETONATE_DELAY2);
	if (flpitch < 10)
		flpitch = 10;
	...
	if (m_hEnemy != NULL)
	{
		...
		pev->velocity = pev->velocity * flAdj + m_vecTarget * 1500;
	}
...
void CSqueakGrenade2::SuperBounceTouch( CBaseEntity *pOther )
{
	...
	// higher pitch as squeeker gets closer to detonation time
	flpitch = 175.0 - 60.0 * ((m_flDie - gpGlobals->time) / SQUEEK_DETONATE_DELAY2);
