...
void CFlockingFlyer :: SpreadFlock( )
{
	...
	while ( pList )
	{
		if ( pList != this && ( pev->origin - pList->pev->origin ).Length() <= AFLOCK_TOO_CLOSE )
		{
			...
			pList->pev->velocity = ( pList->pev->velocity + vecDir ) * 1.5;
...
BOOL CFlockingFlyer :: FPathBlocked( )
{
	...
	// extra wide checks
	UTIL_TraceLine(pev->origin + gpGlobals->v_right * 12, pev->origin + gpGlobals->v_right * 22 + gpGlobals->v_forward * AFLOCK_CHECK_DIST, ignore_monsters, ENT(pev), &tr);
	if (tr.flFraction != 1.0)
	{
		m_flLastBlockedTime = gpGlobals->time;
		fBlocked = TRUE;
