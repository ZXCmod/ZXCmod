...
void CFuncTankMortar::Fire( const Vector &barrelEnd, const Vector &forward, entvars_t *pevAttacker )
{
	if ( m_fireLast != 0 )
	{
		...
		if ( bulletCount > 0 )
		{
			...
			//ExplosionCreate( tr.vecEndPos, pev->angles, edict(), pev->impulse, TRUE ); //deleted
			
		//NOW I FIXED THIS FUCKING BUG WITH TANK (crossfire towers). Congratulations to me ))!

		::RadiusDamage( tr.vecEndPos, pev, pevAttacker, pev->impulse*2, pev->impulse, CLASS_NONE, DMG_FREEZE  ); //not very good idea
		...
		MESSAGE_BEGIN( MSG_BROADCAST, SVC_TEMPENTITY, pev->origin );
			...
			WRITE_BYTE( pev->impulse  ); // scale * 10
		...
		MESSAGE_BEGIN( MSG_BROADCAST, SVC_TEMPENTITY );
					 ...
					 WRITE_BYTE( 30 ); // life
					 WRITE_BYTE( RANDOM_LONG(10,20) ); // width
					 WRITE_BYTE( 0 ); // noise
					 WRITE_BYTE( RANDOM_LONG(100,255) ); // r, g, b
					 WRITE_BYTE( 0 ); // r, g, b
					 WRITE_BYTE( 0 ); // r, g, b
					 WRITE_BYTE( RANDOM_LONG(100,255) ); // brightness
					 WRITE_BYTE( 0 ); // speed?
		MESSAGE_END(); 
